    ...
    {(list || []).length > 0 ? (
      list.map((item: string, i: number) => (
        <span
          ...
          text-xs font-semibold shadow-md hover:shadow-lg hover:-translate-y-1 transition-transform duration-300 text-left`}
        >
          {item}
        </span>
      ))
    ) : (
      <p className="text-gray-400 italic text-left">No data added yet</p>
    )}
  </div>
);
